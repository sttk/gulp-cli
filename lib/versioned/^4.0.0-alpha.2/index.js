...
var colorTable = require('../../shared/log/colorTable');
...
function execute(opts, env) {
  ...
  process.nextTick(function() {
    ...
    if (opts.tasks) {
      ...
      tree.label =
        colorTable.tasklist.tree.title('Tasks for ') +
        colorTable.tasklist.tree.gulpfile(tildify(env.configPath));

      return logTasks(tree, opts.depth, colorTable, function(taskname) {
        return gulpInst.task(String(taskname.toString()));
      });
    }
    if (opts.tasksJson) {
      tree = gulpInst.tree({ deep: true });
