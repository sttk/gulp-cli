...
var colors = require('../customize/colors').customized.tasks.tree;
...
function logTasks(tree, depth, getTask) {
  ...
  log.info(colors.title.text('Tasks for ') + colors.title.gulpfile(tree.label));
...
function printTreeList(lines, spacer, lineInfos) {
  lines.forEach(function(branch, index) {
    ...
    var line = colors.branch(branch);

    if (info.type === 'top') {
      line += colors.task.name(info.name);
      if (info.desc.length > 0) {
        line += spacer(index) + colors.task.description(info.desc);
      }
    } else if (info.type === 'option') {
      line += colors.flag.name(info.name);
      if (info.desc.length > 0) {
        line += spacer(index) + colors.flag.description('…' + info.desc);
      }
    } else { // If (info.type === 'child') {
      line += colors.child.name(info.name);
    }
