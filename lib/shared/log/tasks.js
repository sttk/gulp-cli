...
function logTasks(tree, depth, colorTable, getTask) {
  ...
  log.info(tree.label);
  printTreeList(lines, spacer, lineInfos, colorTable);
}
...
function copyTree(tree, depth, getTask, entryObserver) {
  ...
  _.sortBy(tree.nodes, function(node) {
    return node.label;
    ...
    var newNode = {
      label: node.label,
      desc: _.isString(task.description) ? task.description : '',
      opts: [],
      nodes: [],
    };
    ...
    if (_.isPlainObject(task.flags)) {
      Object.keys(task.flags).sort().forEach(function(flag) {
        ...
        var opt = {
          label: flag,
          desc: _.isString(task.flags[flag]) ? task.flags[flag] : '',
        };
...
function printTreeList(lines, spacer, lineInfos, colorTable) {
  lines.forEach(function(branch, index) {
    var info = lineInfos[index];

    var line = colorTable.tasklist.tree.branch(branch);

    if (info.type === 'top') {
      line += colorTable.tasklist.tree.task.name(info.name);
      if (info.desc.length > 0) {
        line += spacer(index) +
          colorTable.tasklist.tree.task.description(info.desc);
      }
    } else if (info.type === 'option') {
      line += colorTable.tasklist.tree.flag.name(info.name);
      if (info.desc.length > 0) {
        line += spacer(index) + '…' +
          colorTable.tasklist.tree.flag.description(info.desc);
      }
    } else { // If (info.type === 'child') {
      line += colorTable.tasklist.tree.child.name(info.name);
    }
